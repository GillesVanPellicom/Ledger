...
const DebtorDetailsPage: React.FC = () => {
  ...
  const [unsettleConfirmation, setUnsettleConfirmation] = useState<{ isOpen: boolean, receiptId: number | null }>({ isOpen: false, receiptId: null });
  ...
  const fetchDetails = useCallback(async () => {
    setLoading(true);
    try {
      const debtorData = await db.queryOne<Debtor>('SELECT * FROM Debtors WHERE DebtorID = ?', [id]);
      setDebtor(debtorData);

      const receiptData = await db.query<Receipt[]>(`
        SELECT
          r.ReceiptID,
          r.ReceiptDate,
          r.PaymentMethodID,
  ...
  const handlePdfSave = async () => {
    ...
    try {
      ...
      if (receiptIds.length === 0) {
        showError(new Error("No receipts to export for the selected status."));
        return;
      }
      ...
      const lineItemsData = await db.query<any[]>(`
  ...
  const columns = [
    ...
    { header: 'Your Total', render: (row: Receipt) => `€${(row.DebtorTotal || 0).toFixed(2)}` },
    {
      header: 'Status',
      render: (row: Receipt) => (
        <span 
