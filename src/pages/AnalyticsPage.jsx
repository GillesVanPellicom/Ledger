...
const AnalyticsPage = () => {
  ...
  const debtBarChartRef = useRef(null);
  ...
  const [debtStats, setDebtStats] = useState({ netBalances: [], totalOwedToMe: 0, totalOwedByMe: 0 });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const prevYearMonthlyResult = await db.query(`SELECT STRFTIME('%m', r.ReceiptDate) as month, SUM(li.LineQuantity * li.LineUnitPrice) as total FROM Receipts r JOIN LineItems li ON r.ReceiptID = li.ReceiptID WHERE STRFTIME('%Y', r.ReceiptDate) = ? GROUP BY month`, [(parseInt(selectedYear) - 1).toString()]);
      const prevDecResult = await db.query(`SELECT SUM(li.LineQuantity * li.LineUnitPrice) as total FROM Receipts r JOIN LineItems li ON r.ReceiptID = li.ReceiptID WHERE STRFTIME('%Y-%m', r.ReceiptDate) = ?`, [`${parseInt(selectedYear) - 1}-12`]);
      ...
      if (debtEnabled) {
        const debtors = await db.query('SELECT DebtorID, DebtorName FROM Debtors WHERE DebtorIsActive = 1');
        const netBalances = [];
        let totalOwedToMe = 0;
        let totalOwedByMe = 0;
        for (const debtor of debtors) {
          const toMeResult = await db.queryOne(`SELECT SUM(li.LineQuantity * li.LineUnitPrice) as total FROM LineItems li JOIN Receipts r ON li.ReceiptID = r.ReceiptID WHERE li.DebtorID = ? AND NOT EXISTS (SELECT 1 FROM ReceiptDebtorPayments rdp WHERE rdp.ReceiptID = r.ReceiptID AND rdp.DebtorID = li.DebtorID)`, [debtor.DebtorID]);
          const toThemResult = await db.queryOne(`SELECT SUM(li.LineQuantity * li.LineUnitPrice) as total FROM LineItems li JOIN Receipts r ON li.ReceiptID = r.ReceiptID WHERE r.OwedToDebtorID = ? AND r.Status = 'unpaid'`, [debtor.DebtorID]);
  ...
  const debtBarChartOption = useMemo(() => ({
    ...
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' }, formatter: (params) => `${params[0].name}: €${params[0].value.toFixed(2)}` },
