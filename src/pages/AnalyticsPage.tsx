...
import { ArrowDownIcon, ArrowUpIcon, CalculatorIcon, ShoppingBagIcon, CurrencyEuroIcon, BanknotesIcon, UserGroupIcon, InformationCircleIcon } from '@heroicons/react/24/solid';
...
const AnalyticsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const monthlyResult = await db.query<{ month: string, total: number }[]>(`SELECT STRFTIME('%m', r.ReceiptDate) as month, SUM(li.LineQuantity * li.LineUnitPrice) as total FROM Receipts r JOIN LineItems li ON r.ReceiptID = li.ReceiptID WHERE STRFTIME('%Y', r.ReceiptDate) = ? GROUP BY month ORDER BY month ASC`, [selectedYear.toString()]);
      const prevYearMonthlyResult = await db.query<{ month: string, total: number }[]>(`SELECT STRFTIME('%m', r.ReceiptDate) as month, SUM(li.LineQuantity * li.LineUnitPrice) as total FROM Receipts r JOIN LineItems li ON r.ReceiptID = li.ReceiptID WHERE STRFTIME('%Y', r.ReceiptDate) = ? GROUP BY month`, [(parseInt(selectedYear) - 1).toString()]);
      const prevDecResult = await db.queryOne<{ total: number }>(`SELECT SUM(li.LineQuantity * li.LineUnitPrice) as total FROM Receipts r JOIN LineItems li ON r.ReceiptID = li.ReceiptID WHERE STRFTIME('%Y-%m', r.ReceiptDate) = ?`, [`${parseInt(selectedYear) - 1}-12`]);
      ...
      const storeResult = await db.query<{ StoreName: string, total: number }[]>(`SELECT s.StoreName, SUM(li.LineQuantity * li.LineUnitPrice) as total FROM Receipts r JOIN LineItems li ON r.ReceiptID = li.ReceiptID JOIN Stores s ON r.StoreID = s.StoreID WHERE STRFTIME('%Y', r.ReceiptDate) = ? GROUP BY s.StoreName ORDER BY total DESC`, [selectedYear.toString()]);
      setStoreSpending(storeResult.map(s => ({ name: s.StoreName, value: s.total })));

      const averagesResult = await db.queryOne<{ receiptCount: number, totalItems: number, totalSpent: number }>(`SELECT COUNT(DISTINCT r.ReceiptID) as receiptCount, SUM(li.LineQuantity) as totalItems, SUM(li.LineQuantity * li.LineUnitPrice) as totalSpent FROM Receipts r JOIN LineItems li ON r.ReceiptID = li.ReceiptID WHERE STRFTIME('%Y', r.ReceiptDate) = ?`, [selectedYear.toString()]);
      ...
      if (debtEnabled) {
        const debtors = await db.query<{ DebtorID: number, DebtorName: string }[]>('SELECT DebtorID, DebtorName FROM Debtors WHERE DebtorIsActive = 1');
        const netBalances: { name: string, value: number }[] = [];
        let totalOwedToMe = 0;
        let totalOwedByMe = 0;
        for (const debtor of debtors) {
          const toMeResult = await db.queryOne<{ total: number }>(`SELECT SUM(li.LineQuantity * li.LineUnitPrice) as total FROM LineItems li JOIN Receipts r ON li.ReceiptID = r.ReceiptID WHERE li.DebtorID = ? AND NOT EXISTS (SELECT 1 FROM ReceiptDebtorPayments rdp WHERE rdp.ReceiptID = r.ReceiptID AND rdp.DebtorID = li.DebtorID)`, [debtor.DebtorID]);
          const toThemResult = await db.queryOne<{ total: number }>(`SELECT SUM(li.LineQuantity * li.LineUnitPrice) as total FROM LineItems li JOIN Receipts r ON li.ReceiptID = r.ReceiptID WHERE r.OwedToDebtorID = ? AND r.Status = 'unpaid'`, [debtor.DebtorID]);
          const totalToMe = toMeResult?.total || 0;
          const totalToThem = toThemResult?.total || 0;
          totalOwedToMe += totalToMe;
          totalOwedByMe += totalToThem;
          if (totalToMe > 0 || totalToThem > 0) {
            netBalances.push({ name: debtor.DebtorName, value: totalToMe - totalToThem });
          }
        }
        setDebtStats({ netBalances: netBalances.sort((a, b) => b.value - a.value), totalOwedToMe, totalOwedByMe });
      }
  ...
  const debtBarChartOption = useMemo(() => ({
    ...
    series: [{
      ...
      data: debtStats.netBalances.map(d => ({ value: d.value, itemStyle: { color: d.value >= 0 ? '#91cc75' : '#ee6666' } })),
    }]
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 pb-12">
      ...
      {debtEnabled && (
        <div className="pt-2">
          ...
          <Card>
            <div className="p-6">
              <CardHeader title="Net Balance per Entity" tooltipText="Shows the net financial position with each entity. Positive values (green) mean they owe you, negative (red) mean you owe them." />
              {loading ? <Spinner /> : <ReactECharts ref={debtBarChartRef} option={debtBarChartOption} theme={theme} style={{ height: '400px' }} notMerge={true} />}
            </div>
          </Card>
          <hr className="my-8 border-gray-200 dark:border-gray-800" />
        </div>
      ...
      <div className="flex items-center justify-end">
        <div className="w-48"><Select value={selectedYear} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setSelectedYear(e.target.value)} options={availableYears.map(y => ({ value: y, label: y }))} /></div>
      </div>

      <Card>
        <div className="p-6">
          <CardHeader title="Monthly Spending" tooltipText={`Total spending for each month in ${selectedYear}.`} />
          {loading ? <Spinner /> : <ReactECharts ref={monthlyChartRef} option={monthlyChartOption} theme={theme} style={{ height: '300px' }} notMerge={true} />}
        </div>
      </Card>
      <Card>
          ...
          <table className="w-full text-sm text-center">
            <thead>
              <tr>
                ...
                {['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'].map(m => (<th key={m} className="p-2 font-medium text-gray-700 dark:text-gray-300 min-w-[60px]">{m}</th>))}
              </tr>
            </thead>
            <tbody className="divide-y dark:divide-gray-800">
              <tr>
                <td className="p-2 text-left font-medium text-gray-600 dark:text-gray-400">
                  <div className="flex items-center gap-1">vs Prev <Tooltip content="Compares spending to the previous month."><InformationCircleIcon className="h-4 w-4 text-gray-400" /></Tooltip></div>
                </td>
                {monthlySpending.map((month, i) => (<td key={i} className="p-2"><TrendIndicator current={month.total} previous={month.prevMonthTotal} /></td>))}
              </tr>
              <tr>
                <td className="p-2 text-left font-medium text-gray-600 dark:text-gray-400">
                  <div className="flex items-center gap-1">vs Year <Tooltip content="Compares spending to the same month in the previous year."><InformationCircleIcon className="h-4 w-4 text-gray-400" /></Tooltip></div>
                </td>
                {monthlySpending.map((month, i) => (<td key={i} className="p-2"><TrendIndicator current={month.total} previous={month.prevYearMonthTotal} /></td>))}
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <div className="p-6">
            <CardHeader title="Spending by Store" tooltipText={`Breakdown of total spending per store for ${selectedYear}.`} />
            {loading ? <Spinner /> : <ReactECharts ref={storeChartRef} option={storeChartOption} theme={theme} style={{ height: '300px' }} notMerge={true} />}
          </div>
        </Card>
        <Card>
          <div className="p-6">
            <CardHeader title="Receipt Averages" tooltipText={`Average values calculated from all receipts in ${selectedYear}.`} />
            <div className="space-y-6 mt-2">
              <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl"><div className="flex items-center gap-3"><div className="p-2 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg"><CurrencyEuroIcon className="h-6 w-6" /></div><span className="font-medium text-gray-600 dark:text-gray-300">Avg. Receipt Total</span></div><span className="text-xl font-bold">€{averages.avgPerReceipt.toFixed(2)}</span></div>
              <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl"><div className="flex items-center gap-3"><div className="p-2 bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-lg"><ShoppingBagIcon className="h-6 w-6" /></div><span className="font-medium text-gray-600 dark:text-gray-300">Avg. Items / Receipt</span></div><span className="text-xl font-bold">{averages.avgItemsPerReceipt.toFixed(1)}</span></div>
              <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl"><div className="flex items-center gap-3"><div className="p-2 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-lg"><CalculatorIcon className="h-6 w-6" /></div><span className="font-medium text-gray-600 dark:text-gray-300">Avg. Price / Item</span></div><span className="text-xl font-bold">€{averages.avgPricePerItem.toFixed(2)}</span></div>
            </div>
          </div>
        </Card>
