...
const ProductSelector: React.FC<ProductSelectorProps> = ({ isOpen, onClose, onSelect }) => {
  ...
  const handleSaveAndSelect = async (newProductId: number) => {
    setIsCreateModalOpen(false);
    if (newProductId) {
      try {
        const query = `
          ...
          WHERE p.ProductID = ?
        `;
        const newProduct = await db.queryOne<Product>(query, [newProductId]);
        if (newProduct) {
          onSelect(newProduct);
        }
      } catch (error) {
  ...
  const columns = [
    ...
    { header: 'Size', width: '15%', render: (row: Product) => row.ProductSize && row.ProductUnitType ? `${row.ProductSize} ${row.ProductUnitType}` : '' },
    {
      header: '',
      width: '10%',
