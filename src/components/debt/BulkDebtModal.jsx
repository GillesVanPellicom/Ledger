...
const BulkDebtModal = ({ isOpen, onClose, receiptIds, onComplete }) => {
  ...
  const [conflictingReceipts, setConflictingReceipts] = useState([]);
  ...
  const startBulkUpdate = async () => {
    const receiptsWithDebt = await db.query(`
  ...
  const processBulkUpdate = async (ignoreIds) => {
