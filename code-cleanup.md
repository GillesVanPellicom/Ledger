...
We can improve the separation of concerns by moving business logic out of the UI components.

-   **Business Logic:** Complex business logic, such as calculating debt summaries or processing form data, should be moved into utility functions or custom hooks.

## 4. Addressing Technical Debt

-   **`db.js` Wrapper:** The `db.js` file uses `window.electronAPI`. This creates a hard dependency on Electron. We should provide a mock or alternative implementation for a web-based environment to make the components more portable and testable.
